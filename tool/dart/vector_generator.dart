...
void main(List<String> args) async {
  ...
  final existingVecFiles = Set<String>.from(
    vecDirectory.listSync().map((e) => basenameWithoutExtension(e.path)),
  );
  ...
  for (final file in svgFiles) {
    ...
    final baseName = basenameWithoutExtension(inputFile);
...
Future<void> _compileVector({required String inputFile, required String outputFile}) async {
  final result = await Process.run(
    'dart',
    ['run', 'vector_graphics_compiler', '-i', inputFile, '-o', outputFile],
  );
